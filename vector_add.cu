...
//5. 1线程块      每个线程块M*N个线程  thread_id = threadIdx.y * blockDim.x + threadIdx.x 
...
int main() {
    ...
    //调用核函数 cpu调用 gpu运行 
    dim3 dim_grid(N / 64);   //一个grid网格包含n / 512个线程块blocks(为了充分利用sm blocks尽可能多)
    dim3 dim_block(64);      //一个线程块block包含 512个线程threads(最多不超过512个)
